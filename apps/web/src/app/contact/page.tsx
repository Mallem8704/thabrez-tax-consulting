...
export default function ContactPage(): JSX.Element {
  ...
  const [activeMapOffice, setActiveMapOffice] = useState<'BENGALURU' | 'KADIRI'>('BENGALURU');

  const bengaluruAddress = `${companyInfo.registeredOffice.line1}, ${companyInfo.registeredOffice.city} - ${companyInfo.registeredOffice.pincode}`;
  const kadiriAddress = `${companyInfo.branchOffice.line1}, ${companyInfo.branchOffice.city} - ${companyInfo.branchOffice.pincode}`;
  const currentMapAddress = activeMapOffice === 'BENGALURU' ? bengaluruAddress : kadiriAddress;

  const currentMapQuery = activeMapOffice === 'BENGALURU'
          ...
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                      ...
                      <div>
                        ...
                        <div className="flex flex-col mt-0.5 font-mono font-medium text-slate-900">
                          {companyInfo.phoneDetails ? (
                            companyInfo.phoneDetails.map((p, i) => (
                              <a
                                key={i}
                                href={`tel:${p.number.replace(/-/g, '')}`}
                                className="hover:text-[#8B3FA8] transition-colors flex items-center gap-1.5"
                              >
                                <span>{p.display}</span>
                                <span className="text-[10px] font-sans px-1.5 py-0.2 rounded bg-slate-100 text-slate-600 font-normal">
                                  {p.label}
                                </span>
                              </a>
                            ))
                  ...
                  <div className="border-t border-slate-100 pt-5 space-y-3">
                    <div className="rounded-xl border border-slate-100 bg-slate-50 p-3.5 space-y-1">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#8B3FA8] flex items-center gap-1">
                        <Building className="h-3 w-3" /> Registered Office (Bengaluru)
                      </p>
                    ...
                    <div className="rounded-xl border border-slate-100 bg-slate-50 p-3.5 space-y-1">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#C43D6B] flex items-center gap-1">
                        <Building className="h-3 w-3" /> Kadiri Branch Office
                      </p>
                      <p className="text-xs font-semibold text-slate-900">
            ...
            <div className="mt-16 space-y-6 border-t border-slate-200 pt-12">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="space-y-1">
                  <h3 className="text-lg sm:text-xl font-bold text-slate-900 flex items-center gap-2 font-display">
                    ...
                    {activeMapOffice === 'BENGALURU' ? 'Bengaluru Registered Office (Main HQ)' : 'Kadiri Branch Office'}
                  </h3>
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-md relative bg-slate-100">
                <iframe
                  title={activeMapOffice === 'BENGALURU' ? 'Bengaluru Registered Office Map' : 'Kadiri Branch Office Map'}
                  src={mapEmbedUrl}
                  width="100%"
                  height="400"
                  style={{ border: 0 }}
